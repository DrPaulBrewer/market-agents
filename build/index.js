  ...
  sendBidsAndAsks() {
    for (let i = 0, l = this.markets.length; i < l; ++i) {
      ...
      if (unitCost > 0) {
        ...
        if (myPrice) this.ask(market, myPrice); // send my price proposal
      }
    }
  }
...
class Pool {
  ...
  trade(tradeSpec) {
    ...
    if (typeof tradeSpec === 'object' && tradeSpec.bs && tradeSpec.goods && tradeSpec.money && Array.isArray(tradeSpec.prices) && Array.isArray(tradeSpec.buyQ) && Array.isArray(tradeSpec.sellQ) && Array.isArray(tradeSpec.buyId) && Array.isArray(tradeSpec.sellId)) {
      if (tradeSpec.bs === 'b') {
