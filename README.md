...
|`.initPeriod(newPeriod)`| called by `Pool.initPeriod()` to start a new period for an agent; Emits event "pre-period" after performing initialization tasks in the description | period | number or object | Starts a new period for this agent.  If period is a number, retains all of the properties of previous period and changes only period.numnber.  If `period` is an object, `this.period` is set to a deep copy of `period`, `period.startTime` sets the agent's wake up time, and `.init(this.period.init)` is called to set inventory or other agent properties. |
